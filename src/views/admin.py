...
def admin_console():
    return render_template("admin/console.html", ver="v24.1.1",
...
def admin_submissions():
    ...
    if query.get("contest_id"):
        if (query.get("contest_id") == "None" or not check_perm(["ADMIN", "SUPERADMIN", "CONTENT_MANAGER"])): 
...
def admin_users():
    ...
    return render_template("admin/users.html", data=data, length=-(-length // 50), perm_list=USER_PERM, disp_perms=disp_perms, page=page)

@api.route("/createteamaccount", methods=["GET", "POST"])
@admin_required
def create_team_account():
    if request.method == "GET":
        return render_template("admin/createteamaccount.html")
    ...
    if not username or (not verify_text(username) and '[' not in username and ']' not in username):
    ...
    db.execute(("INSERT INTO users(username, password, email, join_date, verified, team_account) "
                "VALUES(:username, :password, :email, datetime('now'), True, True)"),
               username=username, password=generate_password_hash(password), email=email)
    uid = db.execute("SELECT id FROM users WHERE username=:username", username=username)
    os.makedirs("metadata/users/" + str(uid), exist_ok = True)
    open('metadata/users/' + str(uid) + '/profile.md', 'w').close()
    open('metadata/users/' + str(uid) + '/profile.html', 'w').close()
...
def update_perms():
    ...
    if len(user) == 0:
        flash("That user doesn't exist", "danger")
        return redirect("/admin/users")
    if user[0]['team_account']:
        flash("Cannot give perms to a team account!")
        return redirect("/admin/users")
    user_id = int(user_id)
...
def createannouncement():
    ...
    if not request.form.get("name") or not request.form.get("description_md"):
    ...
    write_file('metadata/announcements/' + str(aid) + '.md', description_md)
    write_file('metadata/announcements/' + str(aid) + '.html', description_html)
...
def editannouncement(aid):
    data = db.execute("SELECT name FROM announcements WHERE id=:aid", aid=aid)
    ...
    write_file('metadata/announcements/' + aid + '.md', new_description_md)
    write_file('metadata/announcements/' + aid + '.html', new_description_html)
...
def preview_homepage():
    ...
    template_type = read_file(app.config['HOMEPAGE_FILE'])[20]
